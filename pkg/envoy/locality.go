...
const (
	localityClusterName     = "/cilium-locality-cluster"
	localityEndpointAddress = "127.0.0.1"
	localityEndpointPort    = 1
)

func getLocalNodeZone(ctx context.Context, localNodeStore *node.LocalNodeStore) (string, error) {
...
// newLocalityClusterLoadAssignment publishes the synthetic local endpoint and its zone to the locality cluster.
func newLocalityClusterLoadAssignment(zone string) *envoy_config_endpoint.ClusterLoadAssignment {
	localityLbEndpoints := &envoy_config_endpoint.LocalityLbEndpoints{
					...
					Address: &envoy_config_core.Address{
						Address: &envoy_config_core.Address_SocketAddress{
							SocketAddress: &envoy_config_core.SocketAddress{
								Address: localityEndpointAddress,
								PortSpecifier: &envoy_config_core.SocketAddress_PortValue{
									PortValue: localityEndpointPort,
								},
							},
						},
					},
	...
	if zone != "" {
		localityLbEndpoints.Locality = &envoy_config_core.Locality{Zone: zone}
	}

	return &envoy_config_endpoint.ClusterLoadAssignment{
		ClusterName: localityClusterName,
		Endpoints:   []*envoy_config_endpoint.LocalityLbEndpoints{localityLbEndpoints},
	}
}
